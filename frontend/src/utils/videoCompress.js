...
export function isHardwareCompressionSupported() {
...
async function estimateVideoBitrate(videoTrack, file, durationSec) {
 ...
 *
 * @param {File} file
 * @param {{ onProgress?: (pct:number)=>void, onPhase?: (text:string)=>void }} cb
...
export async function compressVideo(file, { onProgress, onPhase } = {}) {
  ...
  try {
    ...
    console.log('[v0] video compression failed, sending original:', err && err.message);
    return file;
  }
}
